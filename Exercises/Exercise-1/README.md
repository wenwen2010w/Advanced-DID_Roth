...
For R, you will need the following packages: `did`, `dplyr`, `fixest`, `bacondecomp`, `here`, and `haven`. For Stata, you will need `csdid`, `drdid`, `reghdfe`, and `ddtiming`. Note that `ddtiming` is not on SSC, but can installed with `net install ddtiming, from(https://raw.githubusercontent.com/tgoldring/ddtiming/master)`. 
